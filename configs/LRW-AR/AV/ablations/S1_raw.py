...
training_dataset = nnet.datasets.LRWAR(
    ...
    prepared_only=True,
    use_arabish=False,  # S1_raw: Use raw Arabic script (not transliteration)
...
if FREEZE_ENCODER_EPOCHS > 0 and TRANSFER_MODE != "scratch":
    ...
    print(f"[Ablation S1_raw] Frozen: {frozen}, Trainable: {kept} for {FREEZE_ENCODER_EPOCHS} epochs")
    ...
    if epoch == FREEZE_ENCODER_EPOCHS and TRANSFER_MODE != "scratch":
        ...
        for p in model.encoder.parameters():
            if not p.requires_grad:
                ...
                unfroze += 1
        if unfroze:
            print(f"[Ablation S1_raw] Unfroze {unfroze} params at epoch {epoch}")
    return res
...
accumulated_steps = 1
eval_training = False
callback_path = "callbacks/LRW-AR/AV/ablations/S1_raw"
verbose_eval = 1
eval_period_epoch = 1
