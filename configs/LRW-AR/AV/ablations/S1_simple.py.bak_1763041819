"""
Ablation Config: S1_3_simple_translit - LRW-AR
Generated by template_generator.py

Experiment Configuration:
- Script Normalization: simple
- Transfer Mode: full
- Freeze Epochs: 5
- Encoder LR Mult: 0.2
- Head LR Mult: 1.0
...
SCRIPT_NORMALIZATION_TYPE = os.environ.get("SCRIPT_NORMALIZATION_TYPE", "simple")
...
# Script Normalization: SIMPLE (character-level transliteration)
...
if FREEZE_ENCODER_EPOCHS > 0 and TRANSFER_MODE != "scratch":
    ...
    print(f"[Ablation S1_3_simple_translit] Frozen: {frozen}, Trainable: {kept} for {FREEZE_ENCODER_EPOCHS} epochs")
    ...
    if epoch == FREEZE_ENCODER_EPOCHS and TRANSFER_MODE != "scratch":
        ...
        for p in model.encoder.parameters():
            if not p.requires_grad:
                ...
                unfroze += 1
        if unfroze:
            print(f"[Ablation S1_3_simple_translit] Unfroze {unfroze} params at epoch {epoch}")
    return res
...
accumulated_steps = 1
eval_training = False
callback_path = "callbacks/LRW-AR/AV/ablations/S1_3_simple_translit"
verbose_eval = 1
eval_period_epoch = 1
