"""
Ablation Config: S1_simple - LipBengal
...
training_dataset = nnet.datasets.LipBengal(
    ...
    subset_seed=42,
    prepared_only=True,
    label_format="simple",  # Use simple transliteration
)
...
if FREEZE_ENCODER_EPOCHS > 0 and TRANSFER_MODE != "scratch":
    ...
    print(f"[Ablation S1_simple] Frozen: {frozen}, Trainable: {kept} for {FREEZE_ENCODER_EPOCHS} epochs")
    ...
    if epoch == FREEZE_ENCODER_EPOCHS and TRANSFER_MODE != "scratch":
        ...
        for p in model.encoder.parameters():
            if not p.requires_grad:
                ...
                unfroze += 1
        if unfroze:
            print(f"[Ablation S1_simple] Unfroze {unfroze} params at epoch {epoch}")
    return res
...
accumulated_steps = 1
eval_training = False
callback_path = "callbacks/LipBengal/AV/ablations/S1_simple"
verbose_eval = 1
eval_period_epoch = 1
