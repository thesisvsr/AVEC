#!/usr/bin/env python3
"""
Create CTC visualizations with REAL LRS2 frames and clean layout (no overlaps).

Usage:
    python scripts/create_ctc_viz_with_real_frames.py
"""
...
def create_clean_ctc_visualization(
    ...
    # Layout coordinates (NO OVERLAP!)
    frame_y_bottom = 0.42  # Frames start at 42% from bottom
    frame_height = 0.46     # Frames height
    ...
    # Frame positioning
    frame_spacing = 0.84 / num_frames
    frame_width = frame_spacing * 0.72
    frame_left_margin = 0.08
    ...
    # Draw frames (positioned HIGH to avoid overlap)
    for i, frame in enumerate(frames_data):
        x_pos = frame_left_margin + i * frame_spacing + frame_spacing * 0.14
    ...
    # Draw arrows and character boxes (CLEARLY BELOW frames)
    ...
    # Separator line
    ax_main.plot([frame_left_margin, 0.92], [separator_y, separator_y],
                 'k--', linewidth=1, alpha=0.3, zorder=1)
    
    # Final decoded word box
    word_box_width = min(0.36, len(word) * 0.045 + 0.12)
    ...
    # Legend box (top left, clear area)
    legend_text = (
        "CTC Alignment:\n"
...
def main():
    print("Creating CTC visualizations with REAL LRS2 frames...")
    print("=" * 70)
    ...
    try:
        ...
        output_path = Path("export/paper_samples/ctc_real_fixed_chips.png")
        ...
        if len(frames) >= 7:
            output_path2 = Path("export/paper_samples/ctc_real_fixed_cooking.png")
            create_clean_ctc_visualization(
        ...
        if len(frames_8) >= 8:
            output_path3 = Path("export/paper_samples/ctc_real_fixed_home.png")
            create_clean_ctc_visualization(
    ...
    print("\nFiles created with REAL frames and CLEAN layout:")
    print("  • ctc_real_fixed_chips.png/pdf")
    print("  • ctc_real_fixed_cooking.png/pdf")
    print("  • ctc_real_fixed_home.png/pdf")
    print("\n✅ NO OVERLAPPING - Clean, professional layout!")
