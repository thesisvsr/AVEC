#!/usr/bin/env python3
"""
Monitor SINGLE-GPU fast training progress.
Shows utilization, throughput, and training metrics.
"""
...
def get_latest_log():
    """Get the most recent single-GPU training log file."""
    log_dir = Path("logs/single_gpu_training")
    if not log_dir.exists():
        return None
    
    log_files = list(log_dir.glob("single_gpu_*.log"))
    if not log_files:
        return None
...
def format_progress(info, gpu_stats):
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("  FAST SINGLE-GPU TRAINING MONITOR")
    lines.append("=" * 80)
    
    # GPU Status
    if gpu_stats:
        lines.append("\n📊 GPU Status (GPU 0 Active):")
        lines.append("-" * 80)
        for gpu in gpu_stats:
            ...
            status = "🟢 ACTIVE" if gpu['index'] == 0 else "⚪ IDLE"
            
            lines.append(
                f"  GPU {gpu['index']} [{status}]: {util_bar} {gpu['util']:3d}% | "
    ...
    if info['avg_speed']:
        lines.append(f"  Speed: {info['avg_speed']:.2f} it/s (Very Fast!)")
        # Estimate time per epoch
        # Assuming ~1371 iterations per epoch
...
def main():
    ...
    print("🔍 Monitoring single-GPU training...")
