#!/usr/bin/env python3
"""
Create CTC visualizations with MAXIMUM spacing - ZERO overlaps.

Usage:
    python scripts/create_ctc_viz_final_clean.py
"""
...
def create_ultra_clean_visualization(
    ...
    """
    Create visualization with MAXIMUM spacing - absolutely NO overlaps.
    """
    num_frames = len(frames_data)
    
    # Figure dimensions - taller to accommodate spacing
    fig_width = max(18, num_frames * 2.2)
    fig_height = 7.0  # Taller figure for more spacing
    
    fig = plt.figure(figsize=(fig_width, fig_height), facecolor='white')
    
    # Layout with MAXIMUM spacing (all elements well separated)
    # Start frames lower to make room for text at top
    frame_y_bottom = 0.35  # Lower position
    frame_height = 0.42    # Smaller frames
    
    arrow_start_y = frame_y_bottom - 0.04
    arrow_end_y = 0.21
    
    char_box_y = 0.14
    char_box_height = 0.055
    ...
    # Title - positioned HIGH with lots of space
    fig.suptitle(title, fontsize=18, fontweight='bold', y=0.97)
    
    # Subtitle with full transcript - with spacing
    fig.text(0.5, 0.91, f'Full transcript: "{full_transcript}"', 
             ha='center', fontsize=10, style='italic', color='#555555')
    
    # Aligned segment note - with spacing from subtitle
    fig.text(0.5, 0.86, f'Aligned word segment: "{word}"',
             ha='center', fontsize=9.5, weight='bold', color='#2E7D9B')
    
    # Draw frames (positioned LOWER to avoid top text)
    ...
    # Draw arrows and character boxes (well below frames)
    ...
    # Final decoded word box (at bottom with space)
    ...
    # Save with tight bbox but padding
...
def main():
    print("Creating ULTRA CLEAN CTC visualizations (ZERO overlaps)...")
    print("=" * 70)
    ...
    try:
        ...
        output_path = Path("export/paper_samples/ctc_paper_chips.png")
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        create_ultra_clean_visualization(
            frames_data=frames,
        ...
        if len(frames) >= 7:
            output_path2 = Path("export/paper_samples/ctc_paper_cooking.png")
            create_ultra_clean_visualization(
        ...
        # Create HOME visualization
        frames_8 = load_lrs2_video_frames(video_path, num_frames=8, start_frame=10)
        if len(frames_8) >= 8:
            output_path3 = Path("export/paper_samples/ctc_paper_home.png")
            create_ultra_clean_visualization(
    ...
    print("=" * 70)
    print("✅ ULTRA CLEAN visualizations created!")
    print("\n🎯 ZERO OVERLAPS - Maximum spacing between all elements!")
    print("\nFiles created:")
    print("  • ctc_paper_chips.png/pdf    ⭐ PERFECT FOR PAPER")
    print("  • ctc_paper_cooking.png/pdf")
    print("  • ctc_paper_home.png/pdf")
    print("\nLocation: export/paper_samples/")
    print("\n✅ Features:")
    print("  • Real LRS2 frames")
    print("  • NO overlapping (all text well-spaced)")
