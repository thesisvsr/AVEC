...
compile_args = ["-O3", "-DKENLM_MAX_ORDER=6", "-fPIC"]

# PyTorch's C++ frontend (torch/torch.h) requires C++17. Earlier we downgraded
# to C++14 trying to chase an unrelated build failure, which produced many
# spurious errors inside torch/nn/*. Restore C++17; OpenFST 1.6.7 & kenlm build
# fine under C++17 in practice.
preferred_cpp_std = "-std=c++17"
compile_args.append(preferred_cpp_std)
ext_libs = []
...
# Parallel compilation monkey patch disabled for stability; default distutils
# compile is used to avoid intermittent missing directory assembler failures.
